...
export function DashboardContent({ now }: { now: string }) {
  ...
  return (
    <AppShell>
      ...
      {summary.isSuccess && readiness ? <><section className="metrics-grid" aria-label="Financial summary">
      ...
      <p className="foundation-note">Based on current local records · Stored on this device · No bank account is connected.</p></> : null}
    </AppShell>
  );
}
