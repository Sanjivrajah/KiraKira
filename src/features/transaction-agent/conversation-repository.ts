...
export interface ConversationStateRepository {
  ...
  findByUser(telegramUserId: string, telegramChatId?: string): Promise<ConversationState | null>;
  save(state: ConversationState): Promise<ConversationState>;
  removeByUser(telegramUserId: string, telegramChatId?: string): Promise<void>;
...
export class LocalConversationStateRepository implements ConversationStateRepository {
  ...
  async findByUser(telegramUserId: string, telegramChatId?: string): Promise<ConversationState | null> {
    return (await this.readAll()).find((state) => state.telegramUserId === telegramUserId && (telegramChatId === undefined || state.telegramChatId === telegramChatId)) ?? null;
  }
  async save(state: ConversationState): Promise<ConversationState> {
    ...
    return withLocalStorageLock(async () => {
      ...
      const index = states.findIndex((item) => item.telegramUserId === parsed.telegramUserId && item.telegramChatId === parsed.telegramChatId);
      if (index === -1) states.push(parsed); else states[index] = parsed;
      await this.store.write(states);
      return parsed;
    });
  }
  async removeByUser(telegramUserId: string, telegramChatId?: string): Promise<void> {
    await this.remove((state) => state.telegramUserId === telegramUserId && (telegramChatId === undefined || state.telegramChatId === telegramChatId));
  }
