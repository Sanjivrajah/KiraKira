...
import {
  MALAYSIA_ADDRESS_STATE_OPTIONS,
  normalizeMalaysiaStateCode,
} from "@/compliance/myinvois/reference-data/malaysia-states";
...
const makeItemId = () =>
  `item_${typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : Date.now()}`;
const documentTypes = [
  ["invoice", "Invoice"],
  ...
  return [
    ...DEMO_CUSTOMERS.map((customer) =>
      partySchema.parse({
        ...
        taxIdentifiers: customer.tin
          ? [{ scheme: "tin", value: customer.tin, issuingCountryCode: "MY" }]
          : [],
        registrationIdentifiers: [
          { scheme: "brn", value: "NA", issuingCountryCode: "MY" },
        ],
...
export function InvoiceBuilder({
  ...
  } = useForm<InvoiceFormValues, unknown, ValidInvoiceFormValues>({
  ...
  const adjustmentDocument = Boolean(
    watched.documentType &&
    !["invoice", "self_billed_invoice"].includes(watched.documentType),
  );
  ...
  const businessInput = businessOnboardingViewModelSchema.safeParse({
    ...
    businessActivityDescription:
      sourceBusiness.businessActivityDescription || "",
  ...
  const recordIssues =
    preparationResult?.allIssues.filter(
      (issue) => issue.category !== "myinvois",
    ) ?? [];
  ...
  const createBuyer = async () => {
    ...
    try {
      if (
        buyerDraft.countryCode === "MY" &&
        !MALAYSIA_ADDRESS_STATE_OPTIONS.some(
          (option) => option.value === buyerDraft.stateCode,
        )
      ) {
        throw new Error(
          "Choose the Malaysian state for this customer address.",
        );
      }
      const prepared = partyEditorToDomain(buyerDraft, {
        id: buyerDraft.id || `party_${Date.now()}`,
        now: new Date().toISOString(),
      });
      const activeBusinessId =
        business?.id ?? (mode === "demo" ? DEMO_BUSINESS.id : "");
      if (!activeBusinessId)
        throw new Error("Your business workspace is still loading.");
      const party =
        mode === "supabase"
          ? buyerDraft.id
            ? await updateSupabaseParty(activeBusinessId, prepared)
            : await createSupabaseParty(activeBusinessId, prepared)
          : prepared;
      const next = [
        ...parties.filter((existing) => existing.id !== party.id),
        party,
      ];
      setParties(next);
      if (mode === "demo")
      ...
      setBuyerError(
        error instanceof Error ? error.message : "Buyer could not be created.",
      );
    }
  };
  ...
  const submit = async (values: ValidInvoiceFormValues) => {
    ...
    if (!activeBusinessId)
      return setError("root", {
        message: "Your business workspace is still loading.",
      });
    const buyer = parties.find((party) => party.id === values.buyerId);
    if (!buyer)
      return setError("buyerId", { message: "Choose or create a buyer." });
    try {
      const domainInput = {
      ...
      const invoiceInput = {
        ...
        buyerTin:
          buyer.taxIdentifiers.find((identifier) => identifier.scheme === "tin")
            ?.value ?? null,
        issueDate: values.issueDate,
        dueDate: values.dueDate,
      ...
      if (mode === "demo") {
        const existing = browserStorage.get<CommercialDocument[]>(
          FRONTEND_STORAGE_KEYS.documents,
          [],
        );
        browserStorage.set(FRONTEND_STORAGE_KEYS.parties, parties);
        browserStorage.set(FRONTEND_STORAGE_KEYS.documents, [
          domain,
          ...existing.filter((document) => document.id !== domain.id),
        ]);
      }
      router.push(
        initialInvoice
          ? `/invoices/${saved.id}?updated=1`
          : "/invoices?created=1",
      );
    } catch (error) {
      setError("root", {
  ...
  return (
    <>
      <PageHeader
        ...
        title={
          initialInvoice ? "Edit draft invoice" : "Create an invoice or note"
        }
        description="Required MyInvois source fields are marked below. Changes are rechecked when the draft is saved."
      />
      <form
        className="invoice-builder-grid"
        noValidate
        onSubmit={handleSubmit(submit)}
      >
        <div className="invoice-form-column">
          <section
            className="panel invoice-form-section"
            aria-labelledby="basic-details-heading"
          >
            <p className="section-kicker">1 · Basic details</p>
            <h2 id="basic-details-heading">Document details</h2>
            <div className="review-form-grid">
              ...
              {adjustmentDocument ? (
                <FormField
                  ...
                  error={errors.originalDocumentReference?.message}
                  label="Original document reference"
                  {...register("originalDocumentReference")}
                />
              ) : null}
            </div>
          </section>
          ...
          >
            <p className="section-kicker">2 · Customer</p>
            <h2 id="buyer-heading">Customer details</h2>
            <div className="buyer-toolbar">
              <SelectField
                ...
                value={watched.buyerId || ""}
                {...register("buyerId")}
              />
              <div className="buyer-toolbar-actions">
                <button
                  aria-expanded={showBuyerEditor && !buyerDraft.id}
                  className="button button-secondary compact-button"
                  onClick={() => {
                    setBuyerDraft(emptyBuyer);
                    setBuyerError("");
                    setShowBuyerEditor(
                      (open) => !open || Boolean(buyerDraft.id),
                    );
                  }}
                  type="button"
                >
                ...
                {selectedBuyer && selectedBuyer.kind !== "general_public" ? (
                  <button
                    className="button button-secondary compact-button"
                    onClick={() => {
                      setBuyerDraft(partyDomainToEditor(selectedBuyer));
                      setShowBuyerEditor(true);
                      setBuyerError("");
                    }}
                    type="button"
                  >
                ...
                <button
                  ...
                  onClick={() =>
                    setValue("buyerId", "party_general_public", {
                      shouldValidate: true,
                    })
                  }
                  type="button"
            ...
            {selectedBuyer ? (
              <p className="structured-party-summary">
                <strong>{selectedBuyer.legalName}</strong>
                <span>
                  {selectedBuyer.taxIdentifiers.find(
                    (identifier) => identifier.scheme === "tin",
                  )?.value || "TIN incomplete"}{" "}
                  · {selectedBuyer.billingAddress?.city || "Address incomplete"}
                </span>
              </p>
            ) : null}
            {showBuyerEditor ? (
              <fieldset className="customer-editor">
                <legend>
                  {buyerDraft.id ? "Edit customer" : "New customer"}
                </legend>
                <div className="review-form-grid">
                  <SelectField
                    id="buyer-kind"
                    ...
                    value={buyerDraft.kind}
                    onChange={(event) =>
                      setBuyerDraft({
                        ...buyerDraft,
                        kind: event.target
                          .value as PartyEditorViewModel["kind"],
                      })
                    }
                  />
                  <FormField
                  ...
                  <FormField
                    ...
                    label="TIN (required)"
                    required
                    value={buyerDraft.tin}
                    onChange={(event) =>
                      setBuyerDraft({ ...buyerDraft, tin: event.target.value })
                    }
                  />
                  <SelectField
                    id="buyer-registration-type"
                    label="Registration type"
                    options={[
                      { value: "brn", label: "BRN" },
                      { value: "nric", label: "NRIC" },
                      { value: "passport", label: "Passport" },
                      { value: "army_number", label: "Army number" },
                    ]}
                    value={buyerDraft.registrationScheme}
                    onChange={(event) =>
                      setBuyerDraft({
                        ...buyerDraft,
                        registrationScheme: event.target
                          .value as PartyEditorViewModel["registrationScheme"],
                      })
                    }
                  />
                  <FormField
                    id="buyer-registration-value"
                    label="Registration value (required)"
                    required
                    value={buyerDraft.registrationValue}
                    onChange={(event) =>
                      setBuyerDraft({
                        ...buyerDraft,
                  ...
                  />
                  <FormField
                    id="buyer-address-line-1"
                    label="Address line 1 (required)"
                    required
                    value={buyerDraft.addressLine1}
                    onChange={(event) =>
                      setBuyerDraft({
                        ...buyerDraft,
                  ...
                  <FormField
                    ...
                    onChange={(event) =>
                      setBuyerDraft({ ...buyerDraft, city: event.target.value })
                    }
                  />
                  <FormField
                    id="buyer-postcode"
                    label="Postcode"
                    value={buyerDraft.postcode}
                    onChange={(event) =>
                      setBuyerDraft({
                        ...buyerDraft,
                ...
                <div className="customer-editor-actions">
                  ...
                  <button
                    ...
                    type="button"
                  >
                    {buyerDraft.id ? "Save changes" : "Save customer"}
                  </button>
                </div>
              </fieldset>
            ) : null}
          ...
          >
            <div className="invoice-section-heading">
              <div>
                <p className="section-kicker">
                  3 and 4 · Items, tax and classification
                </p>
            ...
            <datalist id="classification-codes">
              <option value="022">Others</option>
              <option value="004">Consolidated e-Invoice</option>
              <option value="036">Self-billed others</option>
            </datalist>
            <datalist id="unit-codes">
              <option value="C62">One</option>
              <option value="KGM">Kilogram</option>
            </datalist>
            <datalist id="tax-type-codes">
              <option value="06">Not applicable</option>
              <option value="02">Service tax</option>
              <option value="01">Sales tax</option>
              <option value="E">Tax exemption</option>
            </datalist>
            <div className="line-items">
              {fields.map((field, index) => (
                <fieldset
                  className="line-item migrated-line-item"
                  key={field.id}
                >
                  ...
                  <FormField
                    ...
                    label="Quantity (required by Niaga)"
                    min="0.01"
                    required
                    step="0.01"
                    type="number"
                  ...
                  <details className="advanced-fields" open>
                    <summary>
                      Required tax and classification fields{" "}
                      <ChevronRight aria-hidden="true" size={16} />
                    </summary>
                    <div className="review-form-grid">
                      <FormField
                        error={
                          errors.items?.[index]?.classificationCode?.message
                        }
                        hint="Enter an official MyInvois classification code."
                        label="Classification code (required)"
                        list="classification-codes"
                        required
                        {...register(`items.${index}.classificationCode`)}
                      />
                      <FormField
                        error={errors.items?.[index]?.unitCode?.message}
                        hint="Enter the applicable unit code."
                        label="Unit code (required by Niaga)"
                        list="unit-codes"
                        required
                        {...register(`items.${index}.unitCode`)}
                      />
                      <FormField
                        error={errors.items?.[index]?.taxTypeCode?.message}
                        hint="Enter an official MyInvois tax type code."
                        label="Tax type code (required)"
                        list="tax-type-codes"
                        required
                        {...register(`items.${index}.taxTypeCode`)}
                      />
                      <FormField
                      ...
                      />
                      <FormField
                        error={errors.items?.[index]?.exemptionReason?.message}
                        label="Exemption reason"
                        {...register(`items.${index}.exemptionReason`)}
                      />
                      <FormField
                        error={errors.items?.[index]?.discountAmount?.message}
                        label="Discount (RM)"
                        min="0"
                        step="0.01"
                        type="number"
                      ...
                      />
                      <FormField
                        error={errors.items?.[index]?.chargeAmount?.message}
                        label="Charges (RM)"
                        min="0"
                        step="0.01"
                        type="number"
                        {...register(`items.${index}.chargeAmount`, {
                          valueAsNumber: true,
                        })}
                      />
                    </div>
          ...
          <details className="panel invoice-form-section progressive-section">
            <summary>5 · Payment</summary>
            <div className="review-form-grid">
              <SelectField
                error={errors.paymentModeCode?.message}
                label="Payment mode"
                options={[
                  { value: "03", label: "Bank transfer" },
                  { value: "01", label: "Cash" },
                ]}
                {...register("paymentModeCode")}
              />
              <FormField
                error={errors.bankAccountIdentifier?.message}
                label="Bank account"
                {...register("bankAccountIdentifier")}
              />
              <FormField
                error={errors.prepaymentAmount?.message}
                hint="Enter a deposit or advance payment already received. Leave as 0 when there was no prepayment."
          ...
          <details className="panel invoice-form-section progressive-section">
            <summary>6 · Additional details</summary>
            <TextareaField
          ...
          <div className="invoice-save-actions">
            ...
            >
              ...
              {initialInvoice ? "Save changes" : "Save document"}
            </button>
          </div>
        </div>
        ...
        <aside className="invoice-preview-column">
          ...
          >
            ...
            <p>
              <strong>Buyer:</strong>{" "}
              {selectedBuyer?.legalName || "Choose a buyer"}
            </p>
            <div className="preview-items">
              {items.map((item, index) => (
                <div className="preview-item-row" key={fields[index]?.id}>
            ...
            <dl className="invoice-totals">
              ...
              {Number(watched.prepaymentAmount) > 0 ? (
                <div>
                  <dt>Prepayment</dt>
                  <dd>
                    −<MoneyDisplay amount={Number(watched.prepaymentAmount)} />
                  </dd>
                </div>
              ) : null}
              <div className="invoice-total">
                ...
                <dd>
                  <MoneyDisplay
                    amount={Math.max(
                      0,
                      totals.total - (Number(watched.prepaymentAmount) || 0),
                    )}
                  />
                </dd>
              </div>
          ...
          <section
            className="readiness-card panel"
            aria-labelledby="readiness-heading"
          >
            <p className="section-kicker">7 · Preparation checks</p>
            <h2 id="readiness-heading">e-Invoice preparation</h2>
            <div
              className="preparation-status-list"
            ...
            >
              ...
              <div>
                <span>e-Invoice preparation</span>
                <strong>
                  {preparationResult
                    ? eInvoiceIssues.length
                      ? `${eInvoiceIssues.length} to fix`
                      : "Niaga checks passed"
                    : "Waiting for details"}
                </strong>
              </div>
              <div>
                <span>MyInvois status</span>
                <strong>Not submitted</strong>
              </div>
            </div>
            {!preparationResult ? (
              <p className="readiness-prerequisite">
                Complete the required invoice fields to run Niaga checks.
              </p>
            ) : null}
            {preparationResult && !preparationResult.allIssues.length ? (
              <div className="preparation-pass">
                <ShieldCheck aria-hidden="true" size={18} />
                <div>
                  <strong>Niaga checks passed</strong>
                  <span>No issues found by the current internal rules.</span>
                </div>
              </div>
            ) : null}
            {preparationResult?.allIssues.length ? (
              <ul className="preparation-issue-list">
                {preparationResult.allIssues.map((issue) => {
                  const target = invoicePreparationFieldTarget(issue);
                  return (
                    <li
                      className={`preparation-issue ${issue.severity}`}
                      key={`${issue.ruleId}-${issue.fieldPath}`}
                    >
                      ...
                      <div>
                        <strong>Niaga check</strong>
                        <p>{issue.message}</p>
                        <small>
                          Why it matters: this field supports an accurate record
                          and e-Invoice preparation.
                        </small>
                        <small>Reference: {issue.sourceReferenceLabel}</small>
                        <button
                          onClick={() => fixPreparationIssue(target)}
                          type="button"
                        >
                          {invoicePreparationFixLabel(issue)}
                        </button>
                      </div>
                    </li>
                  );
            ...
            <p className="readiness-disclosure">
              <strong>MyInvois status: Not submitted.</strong> These are Niaga’s
              internal preparation checks, not official MyInvois validation.
            </p>
          </section>
        </aside>
      </form>
