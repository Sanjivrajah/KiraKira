...
import type { EInvoiceCandidate, EInvoicePreparationStatus, EInvoicePreparationView, EInvoiceSubmissionHistoryFilter, EInvoiceSubmissionRecord, MyInvoisStructuredError, PreparationSupplementalFields } from "@/application/e-invoices";
...
function displayedFieldValue(record: EInvoicePreparationView, candidate: EInvoiceCandidate | undefined, fields: PreparationSupplementalFields, key: keyof PreparationSupplementalFields) {
  if (fields[key] !== undefined) return fields[key];
  const saved = fieldsFrom(record)[key];
  if (saved !== undefined) return saved;
  // The source issue time is already part of the invoice. Show it without
  // creating a document-only override unless the owner changes this field.
  return key === "documentOnlyIssueTime" ? candidate?.issueTime ?? "" : "";
}
