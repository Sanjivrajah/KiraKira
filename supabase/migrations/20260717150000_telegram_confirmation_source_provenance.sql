-- Telegram draft source names are transport-specific; map them to the
-- canonical transactions.source_provenance enum before inserting.
...
begin
  ...
  insert into public.transactions (id, business_id, direction, transaction_type, lifecycle, transaction_date, accounting_date, description, category_code, currency, subtotal_minor, total_minor, payment_status, payment_method_code, e_invoice_treatment, source_provenance, external_key, confirmed_at, confirmed_by, confirmation, source_links, lines, totals, created_by, updated_by)
    values ((p_transaction->>'id')::uuid, v_account.business_id, p_transaction->>'direction', p_transaction->>'transactionType', 'confirmed', (p_transaction->>'transactionDate')::date, (p_transaction->>'transactionDate')::date, p_transaction->>'description', coalesce(nullif(btrim(p_transaction->>'category'), ''), 'uncategorized'), 'MYR', (p_transaction->>'amountMinor')::bigint, (p_transaction->>'amountMinor')::bigint, 'not_applicable', nullif(p_transaction->>'paymentMethod', 'unknown'), 'undetermined', p_transaction->>'sourceProvenance', 'telegram:' || p_account_id || ':' || p_draft_id, public.current_timestamp_utc(), v_account.user_id, p_transaction, '[]'::jsonb, '[]'::jsonb, '{}'::jsonb, v_account.user_id, v_account.user_id)
